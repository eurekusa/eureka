...
class ProductLaunch(FormalTemplateInterface):
    ...
    def build_dashboard(self):
        ...
        for level in ['Country level', 'Family level', 'Molecule level', 'Brand level']:
            if self.clean_data.attrs['columns_dict'][level] is not None:
                ...
                switch = daq.ToggleSwitch(id={'index': 'sales_toggle', 'type': level},
                                          value=False if len(
                ...
                tab = dbc.Container(
                    children=[dbc.Row(
                        [dbc.Col(html.H6('Value'), className="mr-auto", width='auto'),
                         ...
                         ], align="center", justify='end')] + self.get_tab[level](self)
                    ,fluid=True)
    ...
    def get_country_tab(self):
        ...
        max_input = len(self.clean_data[self.clean_data.attrs['columns_dict']['Country level']].unique())
        header_cards = dbc.Row([
            dbc.Col(dbc.Card([
                dbc.CardHeader(html.H5(id='MS_country_header', children='<Market Size>')),
                dbc.CardBody(
                    [
            ...
            ], color="primary", outline=True),className='m-2'),
            dbc.Col(dbc.Card([
                dbc.CardHeader(html.H5(id='cagr_country_header')),
                dbc.CardBody(
                    [
                        html.H5(id='cagr_country', children="<-- the value generated by the call back-->",
                                className="card-title"),
                    ]
                ),
            ], color="primary", outline=True),className='m-2'),
            dbc.Col(dbc.Card([
                dbc.CardHeader([dbc.Row(html.H5(id='RS_country_header')),dbc.Row([dbc.Col(html.P("Top: ",className='m-0 p-0 mr-1'),width='auto'),
                                                                                           dbc.Col(dbc.Input(type="number",
                                                                                                    ...
                                                                                                    size='sm',
                                                                                                     ...
                                                                                                     step=1),width='auto'), ],align="center", justify='start',className="g-0")]),
